...
public class AcmerDao {
    ...
    private static Map<String, Acmer> AllAcmers = new HashMap(){
        {
            ...
            put("myob-_-",new Acmer("bacali", "nasreddine.bacali@gmai.com", "nesrine", "sghaier","Tunisia", "1300", "1500", 22,40, 220, Role.ADMIN));
        }
    };
    ...
    public void addAcmerByHandle(String handle){
        ...
        try {
            ...
            result = objectMapper.readValue(new URL("http://codeforces.com/api/user.info?handles=" + handle), JsonResult.class);
            for (int i = 0; i < result.getResult().size(); i++) {
                System.out.println(result.getResult());
            }
//            this.AllAcmers.put(handle,acmer);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
